...
a.maxSize/e+")"}));if(f.outerHeight()>b(window).height()){var c=b(window).scrollTop(),data_fo_change=m.outerHeight(),n=f.offset().top,k=f.outerHeight(),e=c-n+b(window).height()/2;0>e&&(e=Math.abs(e));0<=e-data_fo_change&&e+data_fo_change<=k?n-c>b(window).height()/2&&(e=data_fo_change):e=c>n+k-data_fo_change?c-n-data_fo_change:c-n+data_fo_change;y(e);b(document).scroll(function(){var a=b(window).scrollTop()-n+b(window).height()/2;0<=a-data_fo_change&&a+data_fo_change<=k&&y(a)})}h.on("destroyed",function(){if(a.onClose&&b.isFunction(a.onClose))a.onClose();h.trigger("close")});b.event.special.destroyed={remove:function(a){a.handler&&
